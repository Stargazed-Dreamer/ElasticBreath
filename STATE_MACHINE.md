...
- 仅屏幕**左上角**参与触发：鼠标在左上角连续停留达到 `CornerHoverSeconds` 即触发一次切换（工作<->休息）。右上角邻近窗口关闭按钮、右下角为"显示桌面"热区、左下角可能是开始菜单，均不响应。
- 触发视觉：鼠标进入左上角时浮现半透明实心圆（圆心在屏幕左上角，用户可见右下角四分之一，半径最大 20px，出现/收回带弹性效果）；悬停期间圆由灰渐变到主题绿，变绿瞬间完成切换；切换后圆保持绿色，鼠标移出左上角后收回。
- 触发后必须先离开左上角区域，才允许下一次触发。
